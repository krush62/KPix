# Shortcuts

|                          | Keyboard            | Keyboard alt     | Mouse                    | Stylus                       | Touch                     |
|--------------------------|---------------------|------------------|--------------------------|------------------------------|---------------------------|
| <h3>General</h3>         |                     |                  |                          |                              |                           |
| New Project              | ctrl + n            |                  |                          |                              |                           |
| Open                     | ctrl + o            |                  |                          |                              |                           |
| Save                     | ctrl + s            |                  |                          |                              |                           |
| Save As                  | ctrl + shift + s    |                  |                          |                              |                           |
| Exit                     | alt + f4            | ctrl + q         |                          |                              |                           |
| Undo                     | ctrl + z            |                  | back button              |                              |                           |
| Redo                     | ctrl + y            | ctrl + shift + z | forward button           |                              |                           |
|                          |                     |                  |                          |                              |                           |
| <h3>Selection</h3>       |                     |                  |                          |                              |                           |
| Copy                     | ctrl + c            |                  |                          |                              |                           |
| Copy Merged              | ctrl + shift + c    |                  |                          |                              |                           |
| Cut                      | ctrl + x            |                  |                          |                              |                           |
| Paste                    | ctrl + v            |                  |                          |                              |                           |
| Paste As New Layer       | ctr + shift + v     |                  |                          |                              |                           |
| Delete                   | del                 |                  |                          |                              |                           |
| Flip Horizontally        | shift + h           |                  |                          |                              |                           |
| Filp Vertically          | shift + v           |                  |                          |                              |                           |
| Rotate                   | ctrl + r            |                  |                          |                              |                           |
| Invert                   | ctrl + i            |                  |                          |                              |                           |
| Select All               | ctrl + a            |                  |                          |                              |                           |
| Deselect                 | ctrl + d            | esc              |                          |                              |                           |
|                          |                     |                  |                          |                              |                           |
| <h3>Tool Selection</h3>  |                     |                  |                          |                              |                           |
| Pencil                   | b                   |                  |                          |                              |                           |
| Shape                    | u                   |                  |                          |                              |                           |
| Fill                     | g                   |                  |                          |                              |                           |
| Select (rectangle)       | m                   |                  |                          |                              |                           |
| Select (wand)            | w                   |                  |                          |                              |                           |
| Eraser                   | e                   |                  |                          |                              |                           |
| Text                     | t                   |                  |                          |                              |                           |
| Spray Can                | s                   |                  |                          |                              |                           |
| Line                     | l                   |                  |                          |                              |                           |
| Curve                    | c                   |                  |                          |                              |                           |
| Stamp                    | p                   |                  |                          |                              |                           |
|                          |                     |                  |                          |                              |                           |
| <h3>Tool Settings</h3>   |                     |                  |                          |                              |                           |
| use tool                 |                     |                  | primary down             | primary down                 | single down               |
| color pick               |                     |                  | secondary down           |                              |                           |
| Selection Add            | shift (hold)        |                  |                          |                              |                           |
| Selection Subtract       | alt + shift (hold)  |                  |                          |                              |                           |
| Selection Intersect      | ctrl + shift (hold) |                  |                          |                              |                           |
| Increase Tool Size       | ctrl + +            |                  | ctrl + mouse wheel up    | hold secondary + hover right |                           |
| Decrease Tool Size       | ctrl + -            |                  | ctrl + mouse wheel down  | hold secondary + hover left  |                           |
| Draw From Center[^1]     | ctrl (hold)         |                  |                          |                              |                           |
| Angle Snap[^1]           | ctrl (hold)         |                  |                          |                              |                           |
| Keep 1:1 Aspect Ratio    | shift (hold)        |                  |                          |                              |                           |
| Line To[^1]              | shift (hold)        |                  |                          |                              |                           |
|                          |                     |                  |                          |                              |                           |
| <h3>Layers</h3>          |                     |                  |                          |                              |                           |
| Switch Visibility        | shift + x           |                  |                          |                              |                           |
| Switch Lock              | shift + l           |                  |                          |                              |                           |
| New Layer                | shift + n           |                  |                          |                              |                           |
| Duplicate                | shift + d           |                  |                          |                              |                           |
| Delete                   | shift + del         |                  |                          |                              |                           |
| Merge Down               | shift + m           |                  |                          |                              |                           |
| Move Layer Up            | shift + ctrl + up   |                  |                          |                              |                           |
| Move Layer Down          | shift + ctrl + down |                  |                          |                              |                           |
| Select Layer Above       | shift + up          |                  | shift + mouse wheel up   |                              |                           |
| Select Layer Below       | shift + down        |                  | shift + mouse wheel down |                              |                           |
|                          |                     |                  |                          |                              |                           |
| <h3>Shading</h3>         |                     |                  |                          |                              |                           |
| Toggle Shading           | space               |                  |                          |                              |                           |
| Toggle Current Ramp Only | x                   |                  |                          |                              |                           |
| Toggle Shading Direction | alt (hold)          |                  |                          |                              |                           |
|                          |                     |                  |                          |                              |                           |
| <h3>Pan & Zoom</h3>      |                     |                  |                          |                              |                           |
| Pan                      |                     |                  | middle down + hover      | secondary down + hover       | two finger down + move    |
| Zoom In                  | +                   |                  | mouse wheel up           | hold secondary + hover up    | two finger pinch increase |
| Zoom Out                 | \-                  |                  | mouse wheel down         | hold secondary + hover down  | two finger pinch decrease |
| Set Optimal Zoom         | Numpad 0            |                  |                          |                              |                           |
| Set Zoom Level 100%      | Numpad 1            |                  |                          |                              |                           |
| Set Zoom Level 200%      | Numpad 2            |                  |                          |                              |                           |
| Set Zoom Level 400%      | Numpad 3            |                  |                          |                              |                           |
| Set Zoom Level 800%      | Numpad 4            |                  |                          |                              |                           |
| Set Zoom Level 1600%     | Numpad 5            |                  |                          |                              |                           |
| Set Zoom Level 3200%     | Numpad 6            |                  |                          |                              |                           |
| Set Zoom Level 4800%     | Numpad 7            |                  |                          |                              |                           |
| Set Zoom Level 6400%     | Numpad 8            |                  |                          |                              |                           |
| Set Zoom Level 8000%     | Numpad 9            |                  |                          |                              |                           |


[^1]: This action is **only** possible with the specified shortcut (hotkey).
